...
def _model(tmp_path, seed, recipe):
    directory = tmp_path / "mappo_jax" / f"seed{seed}"
...
@pytest.mark.parametrize("name", ["cotraining_mappo", "cotraining_mappo_env_diversity"])
def test_mappo_pipeline_uses_one_final_cross_seed_opponent(tmp_path, monkeypatch, name):
...
def test_launcher_pairs_final_models_cyclically_after_training(tmp_path, failed_seed):
...
if "--tag" in args:
    ...
    if args[args.index("--seed") + 1] == os.environ["TEST_FAILED_SEED"]:
        sys.exit(7)
    run = Path(os.environ["JAXBORG_EXP_DIR"]) / "mappo_jax" / tag
...
"""
    ...
    env = {
        **os.environ,
        ...
        "JAXBORG_RECIPES": "cotraining_mappo cotraining_mappo_env_diversity",
        "GPU_COUNT": "2",
        "TEST_CALLS": str(calls_file),
        "TEST_FAILED_SEED": failed_seed,
    }
    result = subprocess.run(
        ["bash", str(ROOT / "scripts/train/run_cotraining.sh")], env=env, capture_output=True, text=True
    )
    ...
    for command in evaluations:
        ...
        assert red.is_file()
        assert red.stem == blue.stem.replace(f"_seed{blue_seed}-", f"_seed{expected[blue_seed]}-")
    if failed_seed:
