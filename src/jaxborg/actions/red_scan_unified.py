...
def apply_scan_unified(
    ...
    # Conditional detection roll: aggressive/stealth consume detection random, basic scan does not
    def _with_roll(s):
        rand_val, next_s = sample_detection_random(s, key)
        detected = success & (rand_val < detection_rate)
        return detected, next_s

    def _without_roll(s):
        return success, s

    detected, state = jax.lax.cond(has_detection_roll, _with_roll, _without_roll, state)
